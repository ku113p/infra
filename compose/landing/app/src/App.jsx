  ...
  useEffect(() => {
    ...
    function handleChange(e) {
      if (!localStorage.getItem("theme")) {
        ...
        if (meta) meta.setAttribute("content", newTheme === "light" ? "#ffffff" : "#09090f");
      }
    }
...
function ContactLinks() {
  return (
    <div className="contact-links">
      ...
      {telegram && (
        <a href={`https://t.me/${telegram}`} className="contact-link" target="_blank" rel="noopener noreferrer" title="Telegram">
          <TelegramIcon />
          <span>@{telegram}</span>
        </a>
      )}
      <a href={`https://github.com/${githubProfile}`} className="contact-link contact-link-github" target="_blank" rel="noopener noreferrer" title="GitHub">
        <GitHubIcon />
        <span>{githubProfile}</span>
      </a>
      {linkedin && (
        <a href={`https://linkedin.com/in/${linkedin}`} className="contact-link" target="_blank" rel="noopener noreferrer" title="LinkedIn">
          <LinkedInIcon />
          <span>LinkedIn</span>
        </a>
      )}
...
function ProjectCard({ project, defaultOpen = false }) {
  ...
  const panelId = `project-${project.name.replace(/\s+/g, "-").toLowerCase()}`;
  return (
    <div className="project-card reveal" ref={ref}>
      <button className="project-header" onClick={() => setOpen(!open)} type="button" aria-expanded={open} aria-controls={panelId}>
      ...
      <div id={panelId} role="region" className={`project-body ${open ? "project-body-open" : ""}`}>
...
function ExperienceCard({ job }) {
  ...
  const ref = useScrollReveal();
  const panelId = `exp-${job.company.replace(/\s+/g, "-").toLowerCase()}-${job.period.slice(-4)}`;
  return (
    <div className="exp-card reveal" ref={ref}>
      <button className="exp-header" onClick={() => setOpen(!open)} type="button" aria-expanded={open} aria-controls={panelId}>
      ...
      <div id={panelId} role="region" className={`exp-body ${open ? "exp-body-open" : ""}`}>
    ...
    <div className="app">
      ...
      <header className="hero">
        <div className="hero-content">
          <img className="hero-avatar" src="/avatar.png" alt={`${name} profile photo`} width="80" height="80" />
